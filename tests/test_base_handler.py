...
class TestSelectAgent:
    ...
    def test_multiple_agents_single_word_match(self):
        """When multiple agents tie on a single keyword, ambiguity detection delegates to orchestrator."""
        routing = {
            "writer": ["content"],
            "marketer": ["content"],
        }
        result = select_agent(routing, "create content")
        # Both score 1 (tied, within 1-point threshold), so orchestrator handles it
        assert result == "orchestrator"
