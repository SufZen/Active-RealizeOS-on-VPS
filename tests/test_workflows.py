...
class TestWorkflowRunner:
    ...
    @pytest.mark.asyncio
    async def test_prompt_node_routes_via_current_router(self, monkeypatch):
        """Prompt nodes should use classify_task + route_to_llm instead of a dead helper."""
        wf = WorkflowDefinition(
            name="test-prompt",
            nodes=[
                WorkflowNode(
                    ...
                    config={"prompt": "Analyze this opportunity"},
                ),
            ],
            entry_node="ask",
        )

        route_to_llm = AsyncMock(return_value="Prompt completed")
        monkeypatch.setattr("realize_core.llm.router.classify_task", lambda message, system_key=None: "reasoning")
        monkeypatch.setattr("realize_core.llm.router.route_to_llm", route_to_llm)

        runner = WorkflowRunner()
        ctx = await runner.execute(wf)

        assert ctx.status == WorkflowStatus.COMPLETED
        assert ctx.results[0]["result"]["output"] == "Prompt completed"
        route_to_llm.assert_awaited_once()
        assert route_to_llm.await_args.kwargs["task_type"] == "reasoning"

    @pytest.mark.asyncio
    async def test_tool_node_failure_marks_workflow_failed(self, monkeypatch):
        """Tool node failures should surface as workflow failures."""
        from realize_core.tools.base_tool import ToolResult
        ...
        wf = WorkflowDefinition(
            ...
            nodes=[
                WorkflowNode(
                    ...
                    config={"tool": "web_search", "params": {"query": "hello"}},
                ),
            ],
            entry_node="run_tool",
        )
        ...
        fake_registry = Mock()
        fake_registry.execute = AsyncMock(return_value=ToolResult.fail("tool exploded"))
        monkeypatch.setattr("realize_core.tools.tool_registry.get_tool_registry", lambda: fake_registry)

        runner = WorkflowRunner()
        ctx = await runner.execute(wf)

        assert ctx.status == WorkflowStatus.FAILED
        assert "tool exploded" in ctx.error
