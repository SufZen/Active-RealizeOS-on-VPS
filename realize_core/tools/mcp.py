...
class MCPServerConnection:
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            ...
            logger.error(f"MCP tool call '{tool_name}' on '{self.name}' timed out after {self.call_timeout}s")
            return f"Error: tool '{tool_name}' timed out after {self.call_timeout}s"
        except Exception as e:
    ...
    async def load_from_config(self, config_path: str | None = None):
        ...
        for name, cfg in config.get("servers", {}).items():
            self.servers[name] = MCPServerConnection(
                ...
                call_timeout=cfg.get("call_timeout", 60),
            )
