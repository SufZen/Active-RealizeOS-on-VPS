...
class ToolRegistry:
    ...
    async def execute(self, action_name: str, params: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult.fail(f"Execution failed: {str(e)[:300]}", duration_ms=duration_ms)

        # Record execution timing
        result.metadata["duration_ms"] = round((time.monotonic() - start) * 1000)

        # Truncate oversized results to prevent token bloat
        if result.output and len(result.output) > self.MAX_RESULT_CHARS:
            result.output = result.output[: self.MAX_RESULT_CHARS] + "\n\n[...truncated]"
    ...
    def check_health(self) -> dict[str, bool]:
        """Check availability of all registered tools. Useful at startup."""
        health = {}
        for name, tool in self._tools.items():
            try:
                health[name] = tool.is_available()
            except Exception as e:
    ...
    def auto_discover(self, package_path: str = "realize_core.tools"):
        ...
        known_modules = [
            ...
            "realize_core.tools.kb_tool",
            "realize_core.tools.voice_tool",
        ]
...
def get_tool_registry() -> ToolRegistry:
    ...
    if not _registry._auto_discovered:
