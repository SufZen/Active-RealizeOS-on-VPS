...
def create_session(
    ...
    If an active session already exists for the same (system_key, user_id),
    it is ended first to prevent orphaned DB records (P0-5).
    """
    existing_key = (system_key, str(user_id))
    if existing_key in _sessions:
        old = _sessions[existing_key]
        if old.stage not in ("completed", "approved"):
            logger.info(f"Ending existing session {old.id} before creating new one")
            end_session(system_key, str(user_id))
