...
def _cleanup_old_backups(prompt_path: Path):
    """Remove oldest backup files if count exceeds MAX_BACKUPS."""
    backups = sorted(prompt_path.parent.glob(f"{prompt_path.stem}.bak.*"), key=lambda p: p.stat().st_mtime)
...
async def apply_prompt_refinement(refinement: dict, kb_path: str = ".") -> str:
    ...
    _cleanup_old_backups(full_path)
    ...
    for change in changes:
        ...
        if change_type == "add":
            ...
            # Find the target section and replace
            old_text = change.get("old_text", "")
            if old_text and old_text in modified:
                modified = modified.replace(old_text, content, 1)
            elif location and location in modified:
                # Try to find by section header
                modified = modified.replace(location, location + "\n" + content, 1)
            else:
                logger.warning(f"Could not find target for modify change, appending instead: {location}")
                modified = modified + "\n\n" + content
    ...
    try:
        ...
        logger.debug(f"Could not clear prompt cache: {e}")
