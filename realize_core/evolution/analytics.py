...
def init_analytics_tables():
    """Create analytics tables if they don't exist. Also runs periodic cleanup."""
    with _get_conn() as conn:
        ...
        """)
        # Add content_hash column if table already exists without it
        try:
            conn.execute("SELECT content_hash FROM evolution_suggestions LIMIT 1")
        except Exception:
            try:
                conn.execute("ALTER TABLE evolution_suggestions ADD COLUMN content_hash TEXT DEFAULT ''")
            except Exception:
                pass
        # Meta table for tracking cleanup frequency
        conn.execute("""
    ...
    # Run periodic cleanup (at most once per 24 hours)
    _maybe_cleanup()


def _maybe_cleanup():
    """Run data retention cleanup if more than 24 hours since last run."""
    try:
        with _get_conn() as conn:
            row = conn.execute(
                "SELECT value FROM _analytics_meta WHERE key = 'last_cleanup'"
            ).fetchone()
            if row:
                last_cleanup = datetime.strptime(row["value"], "%Y-%m-%d %H:%M:%S")
                hours_since = (datetime.now() - last_cleanup).total_seconds() / 3600
                if hours_since < 24:
                    return
            cleanup_old_data()
    except Exception as e:
        logger.debug(f"Cleanup check failed (non-fatal): {e}")


def cleanup_old_data(interaction_days: int = 90, suggestion_days: int = 90):
    """Delete interaction and suggestion records older than the retention period."""
    try:
        with _get_conn() as conn:
            result = conn.execute(
                "DELETE FROM interaction_log WHERE timestamp < datetime('now', ?)",
                (f"-{interaction_days} days",),
            )
            deleted_interactions = result.rowcount

            result = conn.execute(
                "DELETE FROM evolution_suggestions WHERE timestamp < datetime('now', ?) AND status != 'pending'",
                (f"-{suggestion_days} days",),
            )
            deleted_suggestions = result.rowcount

            # Update cleanup timestamp
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            conn.execute(
                "INSERT OR REPLACE INTO _analytics_meta (key, value) VALUES ('last_cleanup', ?)",
                (now,),
            )

            if deleted_interactions or deleted_suggestions:
                logger.info(
                    f"Data retention cleanup: {deleted_interactions} interactions, "
                    f"{deleted_suggestions} suggestions removed"
                )
    except Exception as e:
        logger.debug(f"Data cleanup failed (non-fatal): {e}")
        ...
        with _get_conn() as conn:
            conn.execute(
                ...
                WHERE user_id = ? AND id = (
                    SELECT MAX(id) FROM interaction_log WHERE user_id = ?
                )
            """,
...
def get_interactions_for_analysis(days: int = 7, limit: int = 500) -> list[dict]:
    """Get interactions within a time window for gap analysis."""
    with _get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM interaction_log WHERE timestamp > datetime('now', ?) "
