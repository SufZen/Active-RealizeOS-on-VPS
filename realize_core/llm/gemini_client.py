...
def _calc_cost_for_model(model: str, input_tokens: int, output_tokens: int) -> float:
    """Calculate cost for a Gemini model using per-model pricing."""
    from realize_core.config import MODELS

    # Pricing per 1M tokens (model_id → (input_rate, output_rate))
    pricing = {
        MODELS.get("gemini_flash", ""): (0.075, 0.30),
        MODELS.get("gemini_pro", ""): (1.25, 5.0),
    }
    in_rate, out_rate = pricing.get(model, (0.075, 0.30))
...
async def call_gemini(
    ...
    try:
        ...
        try:
            ...
            cost = _calc_cost_for_model(model, input_tokens, output_tokens)
...
async def call_gemini_pro(system_prompt: str, messages: list[dict], **kwargs) -> str:
    """Convenience wrapper for Gemini Pro (tier 2: reasoning, content, multimodal)."""
    from realize_core.config import MODELS

    return await call_gemini(system_prompt, messages, model=MODELS["gemini_pro"], **kwargs)
