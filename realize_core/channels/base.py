...
class BaseChannel(abc.ABC):
    ...
    # Set to False in subclasses that don't need humanized output (e.g., webhooks)
    ...
    async def handle_incoming(self, message: IncomingMessage) -> OutgoingMessage:
        ...
        Humanization is applied automatically unless humanize_output is False.
        ...
        # Humanize output for human-facing channels
        if self.humanize_output:
            try:
                from realize_core.utils.humanizer import clean_output

                response_text = clean_output(response_text, channel=self.channel_name)
            except Exception as e:
                logger.debug(f"Humanization failed: {e}")
