...
async def list_skills(system_key: str, request: Request):
    ...
    skills = get_skills_for_system(system_key, kb_path=request.app.state.kb_path)
    return {
        "system_key": system_key,
        "skills": [
...
async def reload_systems(request: Request):
    ...
    from realize_core.prompt.builder import clear_cache, warm_cache
    from realize_core.skills.detector import reload_skills

    config = load_config()
    kb_path = Path(config.get("kb_path", request.app.state.kb_path or KB_PATH)).resolve()
    request.app.state.config = config
    request.app.state.kb_path = kb_path
    request.app.state.shared_config = config.get(
        "shared",
        {
    ...
    request.app.state.features = get_features(config)
    request.app.state.systems = build_systems_dict(config, kb_path=kb_path)

    clear_cache()
    reload_skills(kb_path=kb_path)
    warm_cache(kb_path, request.app.state.systems, request.app.state.shared_config)
